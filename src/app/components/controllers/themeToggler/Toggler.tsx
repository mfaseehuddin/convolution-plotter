import { useContext, useEffect } from "react";
import { AppContext } from "../../../context/app/appContext";
import { LightBulbIcon as LightBulbOn } from "@heroicons/react/24/solid";
import { LightBulbIcon as LightBulbOff } from "@heroicons/react/24/outline";

type Props = {};

export default function Toggler({}: Props) {
    const { darkMode, setDarkMode } = useContext(AppContext);

    if (!setDarkMode) {
        throw new Error("setDarkMode is undefined");
    }

    return (
        <label className="relative inline-flex items-center my-1 cursor-pointer">
            <input
                type="checkbox"
                checked={darkMode}
                className="sr-only peer"
                onChange={() => setDarkMode(!darkMode)}
            />
            {/* {darkMode ? (
                <LightBulbOn className="w-4 h-4 text-gray-300" />
            ) : (
                <LightBulbOff className="w-4 h-4 text-black-300" />
            )} */}
            <div
                className={`w-11 h-6 bg-gray-200 rounded-full peer peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600
                relative flex items-center transition-colors duration-200 ease-in-out
            `}
            >
                {darkMode ? (
                    <LightBulbOn className="w-4 h-4 text-gray-300 translate-x-1" />
                ) : (
                    <LightBulbOff className="w-4 h-4 text-black-300 translate-x-6" />
                )}
            </div>
            {/* <span className="ml-3 text-sm font-medium text-gray-900 dark:text-gray-300">
                {darkMode ? "Dark mode" : "Light mode"}
            </span> */}
        </label>
    );
}
